...
impl Policy {
    ...
    pub fn extract_pub_key(&self) -> Result<VerifyingKey<p256::NistP256>, anyhow::Error> {
        let cert = base64::decode(&self.signatures[0].cert).unwrap();
        ...
        VerifyingKey::<p256::NistP256>::from_public_key_der(&pub_key_bytes[..]).map_err(|e| anyhow!("Cannot load key: {:?}", e))
    }
...
mod tests {
    ...
    fn validate_signature_success() {
        ...
        let outcome = policy.verify_signature(&pub_key.unwrap(), &msg);
    ...
    fn validate_signature_failure() {
        ...
        let good_policy = Path::new(CRATE).join("tests/test_data/policy_bad.json");
        let raw_json = read(good_policy).expect("Cannot read bad policy file");
        let raw_policy: RawPolicy = serde_json::from_slice(&raw_json).expect("Could not create Raw Policy");
